...
{A} assigned Dragonlord Ojutai (24) to attack {D}.
...
def test_stdout_continuations() -> None:
    ...
    # 5 block declarations (Power Cosmic's two lines were one Forge entry).
    assert combat == [
        f"{A} assigned Sunscorch Regent (75) to attack {C}.",
        f"{A} assigned Dragonlord Ojutai (24) to attack {D}.",
    ...
    assert resolves[0]["more"] == ["• Destroy all artifacts.",
                                   "• Destroy all creatures with mana value 4 or greater."]
    # A crash dump is never data: not an event, not attached to one.
    dmg = [e for e in evs if e["action"] == "damage"]
    assert len(dmg) == 1 and "more" not in dmg[0], dmg
    assert not any("NullPointer" in json.dumps(e) for e in evs)
    # The event count itself is what the old parser got wrong.
    assert len(evs) == 14, [(e["action"], e["raw"][:40]) for e in evs]
    print("  stdout: 3 attack + 4 block lines kept where 1 + 3 used to survive")


def _shim_jsonl(split: bool) -> str:
    """The playtester's turn as the shim writes it. `split=False` is the
    pre-fix reading, kept only to prove readapt recovers the events."""
    ...
    recs = [
        ...
        {"rec": "zone", "game": 0, "turn": 18, "phase": "COMBAT_DAMAGE", "card": "Thopter Token",
         ...
         "toPlayer": D, "types": "Artifact Creature", "pt": "1/1", "token": True},
...
def test_shim_continuations() -> None:
    r = parse_shim_jsonl(_shim_jsonl(split=True), source="unit-test")
    g = r["games"][0]
...
def test_readapt_recovers_dropped_combat_lines() -> None:
    ...
    raw = _shim_jsonl(split=True)
    with tempfile.TemporaryDirectory() as d:
        tmp = Path(d)
        (tmp / "shim_raw_run77_rot0.jsonl").write_text(raw, encoding="utf-8")
        full = parse_shim_jsonl(raw, source="fixture")
        # Simulate the old adapter: drop the continuation event (Ojutai's
        # attack) and the attached modal text, keeping everything else.
        for g in full["games"]:
            for t in g["turns"]:
                t["events"] = [e for e in t["events"]
                               if "Dragonlord Ojutai (24) to attack" not in e["raw"]]
                for e in t["events"]:
                    e.pop("more", None)
        ...
        combat = [e["raw"] for t in after["games"][0]["turns"] for e in t["events"]
                  if e["action"] == "combat"]
        assert f"{A} assigned Dragonlord Ojutai (24) to attack {D}." in combat, combat
    ...
    print("  readapt: recovers the dropped attack as +1 events with the fingerprint intact")
